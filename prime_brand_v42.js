(function(){
  ...
  function renderBrand(){
    ...
    if(header){header.className='mark prime-logo-header';header.innerHTML='<img alt="Prime">';setLogo(header.querySelector('img'))}
    var nav=document.querySelector('.navbrand');
    if(nav){nav.innerHTML='<img class="prime-logo-nav" alt="Prime">';setLogo(nav.querySelector('img'))}
    ...
    if(login){
      ...
      if(current){var img=document.createElement('img');img.className='prime-logo-login';current.replaceWith(img);setLogo(img)}
      var title=login.querySelector('h2');if(title)title.style.setProperty('display','none','important');
    }
  }

  var style=document.createElement('style');
  style.id='primeBrandCompatV42';
  ...
  if(typeof baseCommercialDoc==='function')window.commercialDoc=function(obj,kind){
    ...
    return html.replace('</style>','.prime-receipt-brand{min-height:100px!important;padding:12px 24px!important}.prime-receipt-logo{display:block;width:135px;max-width:31%;height:auto;object-fit:contain;background:transparent!important}@media(max-width:650px){.prime-receipt-brand{min-height:82px!important;padding:10px 14px!important}.prime-receipt-logo{width:115px;max-width:50%}}@media print{.prime-receipt-brand{min-height:92px!important}.prime-receipt-logo{width:125px}}</style>');
  };
