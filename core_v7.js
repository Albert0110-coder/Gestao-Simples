...
var CACHE_KEY='gestaoMercosLikeV1';
var session=null;
var db={products:[],customers:[],suppliers:[],sales:[],buys:[]};
var currentCustomerId='',editCustomerId='',editProductId='',editSupplierId='',syncTimer=null;
...
function arrays(){['products','customers','suppliers','sales','buys'].forEach(function(k){if(!Array.isArray(db[k]))db[k]=[]})}
...
async function doLogin(){
  ...
  try{
    ...
    $('loginMsg').textContent=e.message||'Não foi possível entrar.';
  }
}
...
async function pull(manual){
  ...
  try{
    ...
    db={products:[],customers:[],suppliers:[],sales:[],buys:[]};Object.assign(db,j.data||{});arrays();cache();renderAll();setSync('Sincronizado','ok');
...
function go(page){
  ...
  var names={home:'Início',orders:'Pedidos',customers:'Clientes',products:'Produtos',more:'Mais'};$('pageTitle').textContent=names[page]||'Gestão Simples';
  if(page==='orders')renderOrders();if(page==='customers')renderCustomers();if(page==='products')renderProducts();
}
window.go=go;
